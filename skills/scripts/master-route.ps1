...
# reverse-skill PRIMARY router. EN + ZH keyword patterns; UTF-8 output.
param(
    [string] $Hint = '',
    [string] $OutDir = ''
)
...
# Lowercase for Latin tokens; keep original for CJK (ToLowerInvariant does not hurt CJK).
...
if ($t -match '\bipa\b|ios.?reverse|objection|mobsf|mobile.?reverse|越狱|ios.?逆向') { [void]$sel.Add('R2') }
...
if ($t -match 'dsl.?vm|fireye|opcode.?vm|custom.?vm|自定义.?虚拟机|opcode.?vm') { [void]$sel.Add('R4') }
...
if ($t -match 'malware|yara|sigma|sandbox|virus.?sample|pe-?sieve|恶意.?软件|病毒.?样本|木马.?分析') { [void]$sel.Add('R9') }
...
if ($t -match 'graphql|bola|bfla|api.?secur|oauth|接口.?安全|越权|未授权.?访问') { [void]$sel.Add('R12') }
if ($t -match 'sbom|supply.?chain|trivy|gitleaks|syft|供应链|依赖.?扫描') { [void]$sel.Add('R13') }
if ($t -match 'llm|prompt.?inject|jailbreak|agent.?secur|garak|owasp.?llm|提示词.?注入|模型.?红队|越狱.?测试') { [void]$sel.Add('R14') }
...
if ($t -match 'reverse|逆向' -and $t -notmatch 'apk|js.?reverse|ios|mobile|\.net|firmware|malware|安卓|固件|恶意') { [void]$sel.Add('R0') }
...
$priority = @('R4','R1','R2','R3','R5','R9','R6','R7','R8','R17','R16','R18','R10','R11','R12','R13','R14','R15','R19','R20','R0')
$primary = $null
